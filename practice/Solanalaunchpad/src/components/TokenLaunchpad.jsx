...
export function TokenLaunch(){
    ...
    async function createToken(){
        ...
        const transaction = new Transaction().add(
            ...
            createInitializeMetadataPointerInstruction(minKeypair,publicKey.wallet.publicKey,minKeypair.publicKey),
        ...
        const transaction3 = new Transaction().add(
            createAssociatedTokenAccountInstruction(mintKeypair,publicKey,associatedToken,wallet.publicKey,1000000000,[],TOKEN_2022_PROGRAM_ID)
        )
    ...
    return(
        <div className="flex flex-col  items-center justify-center min-h-screen bg-gray-50">
            ...
            <div className="space-y-4">
                <input type="text" className="inputText px-4 py-2 border rounded-md w-80" placeholder="Name" />
                <input type="text" className="inputText px-4 py-2 border rounded-md w-80" placeholder="Symbol" />
                <input type="text" className="inputText px-4 py-2 border rounded-md w-80" placeholder="Image Url" />
                <input type="text" className="inputText px-4 py-2 border rounded-md w-80" placeholder="Initial Supply" />


            </div>
            <button onClick={createToken} className="btn btn-primary mt-4 px-6 py-2 bg-blue-600 text-white rounded-lg">Create Token</button>
        </div>
    )
}